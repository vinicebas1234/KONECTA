...
class DetectorMaos:
    ...
    def desenhar(self, frame_bgr, res):
    ...
    def extrair_features(self, res):
        ...
        for idx, hand_lms in enumerate(res.multi_hand_landmarks):
            ...
            # Normalização simples: centralizar no pulso (0) e escalar
            center = pts[0].copy()
            pts = pts - center
            m = np.max(np.abs(pts))
            if m > 0:
...
class LibrasApp(tk.Tk):
    ...
    def _loop_camera(self):
        ...
        while self.camera_rodando:
            ...
            tem_mao = res.multi_hand_landmarks is not None
